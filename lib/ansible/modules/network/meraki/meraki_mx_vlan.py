...
def main():
    ...
    argument_spec.update(state=dict(type='str', choices=['absent', 'present', 'query'], default='query'),
                         ...
                         appliance_port_id=dict(type='int'),
                         type=dict(type='str', choices=['access', 'trunk']),
                         enabled=dict(type='bool', default=True),
                         drop_untagged=dict(type='bool', default=False),
                         accessPolicy=dict(type='str', choices=['open', '8021x-radius', 'mac-radius', 'hybrid-radius'], default='open']),
    ...
    meraki = MerakiModule(module, function='mxVlan')

    meraki.params['follow_redirects'] = 'all'

    query_urls = {'mxVlan': '/networks/{net_id}/appliancePorts'}
    query_url = {'mxVlan': '/networks/{net_id}/appliancePorts/{appliance_port_id}'}
    create_url = {'mxVlan': '/networks/{net_id}/appliancePorts'}
    update_url = {'mxVlan': '/networks/{net_id}/appliancePorts/'}
    delete_url = {'mxVlan': '/networks/{net_id}/appliancePorts/'}
    ...
    if meraki.params['state'] == 'query':
        if not meraki.params['appliance_port_id']:
            meraki.result['data'] = get_vlans(meraki, net_id)
        else:
            path = meraki.construct_path('get_one', net_id=net_id, custom={'appliance_port_id': meraki.params['appliance_port_id']})
            response = meraki.request(path, method='GET')
            meraki.result['data'] = response
    elif meraki.params['state'] == 'present':
        payload = {'number': meraki.params['appliance_port_id'],
                   'vlan': meraki.params['vlan_id'],
                   'type': meraki.params['type'],
                   'enabled': meraki.params['enabled'],
                   }
        path = meraki.construct_path('get_one', net_id=net_id, custom={'appliance_port_id': meraki.params['appliance_port_id']})
        original = meraki.request(path, method='GET')
        if payload['type'] == 'access':
            payload['accessPolicy'] = meraki.params['accessPolicy']
